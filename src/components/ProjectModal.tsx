...
import { X, ExternalLink, Code2, Check, ShieldCheck, Activity, Cpu, Server, Database, Globe } from 'lucide-react';
...
export const ProjectModal: React.FC<ProjectModalProps> = ({ project, mode, onClose }) => {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'telemetry' | 'architecture'>('overview');
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800 bg-[#0d1117]/95">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-base sm:text-lg font-bold text-white tracking-wide">
              {project.title} {mode === 'github' ? '— Source Repository' : '— Live Environment'}
            </h3>
          </div>
          <button
            onClick={onClose}
            ...
            <div className="space-y-6">
              ...
              {project.metrics && (
                <div className="p-4 rounded-xl bg-cyan-950/30 border border-cyan-500/20 flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-xs font-mono text-cyan-300 uppercase">Benchmark & Performance</div>
                    <div className="text-sm font-semibold text-white mt-0.5">{project.metrics}</div>
                  </div>
                </div>
              )}
          ...
          {activeTab === 'architecture' && (
            <div className="space-y-4 font-mono text-xs">
              <div className="bg-[#0d1117] p-4 rounded-xl border border-slate-800 space-y-3">
                <div className="text-cyan-400 font-bold">// System Components</div>
                <div className="space-y-2 text-slate-300">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span>Distributed Persistent Layer (PostgreSQL with read-replicas + Redis cluster)</span>
                  </div>
                </div>
              </div>

              <div className="bg-[#0d1117] p-4 rounded-xl border border-slate-800 space-y-2 text-slate-400">
                <div className="text-emerald-400 font-bold">// Production Release Pipeline</div>
                <p className="text-xs leading-relaxed">
                  Automated linting, matrix test suites, containerized Docker builds, and zero-downtime rolling deployments via Kubernetes helm charts.
                </p>
              </div>
            </div>
          )}
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-3 gap-3 text-center">
                ...
                <div className="p-4 rounded-xl bg-[#0d1117] border border-slate-800">
                  ...
                  <div className="text-cyan-300 font-bold text-sm mt-1">24ms avg</div>
                </div>
                <div className="p-4 rounded-xl bg-[#0d1117] border border-slate-800">
                  <div className="text-xs font-mono text-slate-400">Availability</div>
                  <div className="text-cyan-300 font-bold text-sm mt-1">99.99%</div>
                </div>
              </div>

              <div className="p-4 rounded-xl bg-[#0d1117] border border-slate-800 font-mono text-xs text-slate-400">
                <div className="text-slate-200 mb-2 font-semibold">// Healthcheck Logs</div>
                <div className="text-emerald-400/90">&gt; [OK] GET /healthz 200 OK (1.2ms)</div>
                <div className="text-emerald-400/90">&gt; [OK] DB Pool healthy: 42 connections idle</div>
                <div className="text-emerald-400/90">&gt; [OK] Memory utilization: 18.4% nominal</div>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-slate-800 bg-[#0d1117]/95">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="px-4 py-2 text-xs font-bold text-slate-950 bg-gradient-to-r from-cyan-400 to-emerald-400 hover:opacity-90 rounded-md transition-all duration-200 hover:-translate-y-0.5 flex items-center gap-1.5 shadow-[0_0_12px_rgba(0,242,254,0.25)] cursor-pointer"
            >
